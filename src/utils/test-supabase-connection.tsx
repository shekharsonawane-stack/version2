...
export async function quickConnectionCheck(): Promise<boolean> {
  try {
    const response = await fetch(`${API_BASE}/health`, {
      ...
      signal: AbortSignal.timeout(15000), // 15 second timeout for cold starts
    });

    const data = await response.json();
    return response.ok && data.status === "ok";
  } catch (error) {
    console.warn("ℹ️ Backend server not available - using offline mode");
    console.info("💡 To enable backend features, deploy using: supabase functions deploy server");
    return false;
  }
}